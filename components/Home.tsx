...
const Home: React.FC<HomeProps> = ({ onPortfolioClick, onContactClick }) => {
  ...
  return (
    <div>
      <section className="relative min-h-[60vh] lg:min-h-[90vh] flex items-center overflow-hidden bg-brand-cream">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10 py-4 lg:py-24">
          <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-center">
            <div className="mb-16 lg:mb-0">
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-brand-navy leading-[1.05] mt-0 mb-8 lg:mt-0 lg:mb-8 animate-fade-up" style={{ animationDelay: '100ms' }}>
                Crafting Spaces<br />
              ...
              <p className="text-lg text-brand-charcoal/60 mb-10 max-w-lg leading-relaxed animate-fade-up" style={{ animationDelay: '200ms' }}>
                From custom kitchens to premium bathroom retreats, we bring high-end craftsmanship and local reliability to every square inch of your home.<br />
                <span className="text-brand-navy font-semibold">Stuhl Services LLC</span> is a local family-owned company built on trust, quality craftsmanship, and exceptional references.
              </p>
              <div className="flex flex-row gap-3 sm:gap-4 animate-fade-up" style={{ animationDelay: '300ms' }}>
                <button 
                  onClick={onContactClick}
                  className="group flex-1 sm:flex-none px-6 sm:px-10 py-4 sm:py-5 bg-brand-navy text-white font-bold rounded-2xl hover:bg-brand-charcoal transition-all duration-300 shadow-lg flex items-center justify-center text-sm sm:text-base tracking-wide"
                >
                  Get a Quote
                  <svg className="ml-2 w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg>
                </button>
                <button 
                  onClick={onPortfolioClick}
                  className="group flex-1 sm:flex-none px-6 sm:px-10 py-4 sm:py-5 bg-brand-gold text-white font-bold rounded-2xl hover:bg-brand-gold/90 transition-all duration-300 shadow-lg flex items-center justify-center text-sm sm:text-base tracking-wide"
                >
                  Our Work
                  <svg className="ml-2 w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg>
                </button>
      ...
      <section className="py-14 lg:py-28 bg-white relative overflow-hidden">
            ...
            {SERVICES.map((service, idx) => {
              ...
              return (
                <FadeInSection key={idx} delay={idx * 100}>
                  <div className="group relative bg-brand-navy/[0.04] p-5 sm:p-6 rounded-2xl border border-brand-navy/10 hover:border-brand-navy/15 transition-all duration-500 hover:shadow-xl hover:shadow-brand-navy/5 hover:-translate-y-1 h-full flex items-start gap-4">
                    <div className={`w-12 h-12 shrink-0 bg-gradient-to-br ${gradient} rounded-xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                      <Icon size={22} />
                    </div>
                    <div>
                      <h3 className="text-base font-extrabold mb-1 text-brand-navy">{service.title}</h3>
      ...
      <section className="py-6 lg:py-10 bg-brand-cream">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          <FadeInSection>
            <div className="bg-gradient-to-b from-brand-navy to-brand-charcoal rounded-[2rem] sm:rounded-[2.5rem] p-8 sm:p-10 lg:p-20 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 w-80 h-80 bg-brand-gold/10 rounded-full -mr-40 -mt-40 blur-3xl"></div>
              <div className="absolute bottom-0 left-1/2 w-[500px] h-[500px] bg-brand-cyan/5 rounded-full -translate-x-1/2 translate-y-1/2 blur-3xl"></div>
              
              <div className="relative z-10 lg:grid lg:grid-cols-2 gap-16 items-start">
                <div>
                  ...
                  <h2 className="text-3xl sm:text-4xl lg:text-5xl mb-6 leading-tight">Expert Service.<br /><span className="text-brand-gold italic">Local Trust.</span></h2>
                  <p className="text-white/50 mb-10 text-base sm:text-lg leading-relaxed">At Stuhl Services, we treat your home like our own. Our business is built on word-of-mouth and a long list of successful local references.</p>
                </div>
                <div>
                  <ul className="space-y-6">
                    {[
                      ...
                      <li key={i} className="flex items-start gap-4">
                        <div className="bg-brand-gold/20 p-1.5 rounded-full mt-0.5 shrink-0">
                          <CheckCircle className="text-brand-gold" size={18} />
                        </div>
                        <span className="text-white/80 font-medium text-sm sm:text-base leading-relaxed">{item}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <button
                    onClick={onContactClick}
                    className="mt-10 px-8 py-4 bg-brand-gold text-white font-bold rounded-2xl hover:bg-brand-gold/90 transition-all duration-300 shadow-lg text-sm sm:text-base tracking-wide"
                  >
                    Free Quote
                  </button>
                </div>

...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Something went wrong. Please call us at (908) 723-5922 or try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="py-14 lg:py-28 bg-brand-cream">
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="bg-white p-8 lg:p-12 rounded-3xl shadow-xl shadow-brand-navy/5 border border-brand-navy/5">
                ...
                <button 
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-brand-navy text-white font-bold py-4 rounded-xl hover:bg-brand-charcoal transition-all duration-300 shadow-lg shadow-brand-navy/20 hover:shadow-xl hover:shadow-brand-navy/30 flex items-center justify-center group text-sm tracking-wide disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {submitting ? 'Sending...' : 'Request Free Estimate'}
                  {!submitting && <Send size={16} className="ml-2 group-hover:translate-x-1 group-hover:-translate-y-0.5 transition-transform" />}
                </button>
              </form>
            )}
          </FadeInSection>
